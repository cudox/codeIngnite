...
import static com.bancodebogota.consultarmoras.utilitarios.ConstantesModelo.LOCALE;
...
public class UtilitariosTexto {
     ...
     */
    public static final String FORMATO_CODIFICACION_CARACTERES_UTF_8 = "UTF-8";
    ...
    /**
     * Obtiene una cadena concatenada consigo misma un número determinado de veces
     ...
     */
    public static String empaquetarLista(List<?> valores, String separador)
    {
        if (valores == null || valores.isEmpty())
            throw new IllegalArgumentException("La lista de valores no puede ser nula ni vacía.\n");
        else if (esVacio(separador))
            throw new IllegalArgumentException("La cadena separarador no puede ser nula ni vacía.\n");
      
        StringBuilder resultado = new StringBuilder();
        for (Object objeto: valores)
        {
            if (objeto == null)
                resultado.append("null"+separador);
            else
                resultado.append(objeto.toString()+separador);
        }
        
        return resultado.substring(0, resultado.length()-separador.length());
    }//empaquetarLista
     ...
     * @param clob Clob a convertir
     * @return String convertido del Clob que llega
     */
